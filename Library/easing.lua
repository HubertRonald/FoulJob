...
easing.inCircular = function(ratio)
	return -(sqrt(1-ratio*ratio)-1)
end
easing.outCircular = function(ratio)
	return sqrt(1-(ratio-1)*(ratio-1))
end
easing.inOutCircular = function(ratio)
	ratio = ratio * 2
	if ratio < 1 then
		return -0.5*(sqrt(1-ratio*ratio)-1)
	else
		ratio = ratio - 2
		return 0.5*(sqrt(1-ratio*ratio)+1)
	end
...
easing.inElastic = function(ratio)
	...
	ratio = ratio - 1
	return -(elasticA * pow(2, 10 * ratio) * sin((ratio - elasticS) * (2 * npi) / elasticP));
end
easing.outElastic = function(ratio)
	if ratio == 0 or ratio == 1 then
		return ratio
	end
	return elasticA * pow(2, -10 * ratio) *  sin((ratio - elasticS) * (2 * npi) / elasticP) + 1;
end
easing.inOutElastic = function(ratio)
	...
	ratio = ratio*2-1
	if ratio < 0 then
		return -0.5 * (elasticA * pow(2, 10 * ratio) * sin((ratio - elasticS*1.5) * (2 * npi) /(elasticP*1.5)));
	end
	return 0.5 * elasticA * pow(2, -10 * ratio) * sin((ratio - elasticS*1.5) * (2 * npi) / (elasticP*1.5)) + 1;
end
easing.inExponential = function(ratio)
	if ratio == 0 then
		return 0
	end
	return pow(2, 10 * (ratio - 1))
end
...
easing.inOutExponential = function(ratio)
	...
	ratio = ratio*2-1
	if 0 > ratio then
		return 0.5*pow(2, 10*ratio)
	end
	return 1-0.5*pow(2, -10*ratio)
end
...
easing.inSine = function(ratio)
	return 1-cos(ratio * (npi / 2))
end
easing.outSine = function(ratio)
	return sin(ratio * (npi / 2))
end
easing.inOutSine = function(ratio)
	return -0.5*(cos(ratio*npi)-1)
end